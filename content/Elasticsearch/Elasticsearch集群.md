...
ES 集群不是简单地“多启动几个节点”。真正需要理解的是：索引如何被拆成分片，分片如何分配到节点，写入和查询如何在主分片、副本分片、协调节点之间流转，以及一些看起来普通的 settings 为什么会影响集群稳定性。

本文先从索引 settings 讲起，再看本地三节点集群如何部署。
...
该配置用于设置 ES 索引的**主分片数量**和**副本数量**。注意，`number_of_replicas` 配置的是每个主分片有几份副本，不是配置“从节点数量”。ES 的副本是 Shard 的拷贝，会被分配到 data 节点上。
...
当写请求到达主分片时，主分片会把同一写操作复制到副本分片，并在满足写入确认条件后返回。主分片挂掉后，符合条件的副本分片会被提升为新的主分片。读请求可以被负载均衡到主分片或副本分片，从而提升查询吞吐。副本参数可以在运行时修改。
