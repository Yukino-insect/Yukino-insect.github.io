...
如果你对“原型”和“传统 class-only 模型”的区别还不清楚，可以先读同目录下的《JavaScript 原型与 class-only 模型：为什么 class 只是入口，不是底层》。这一篇会继续讲对象、类和容器的使用边界；那一篇专门解释底层对象模型。把两个问题混在一起理解，通常只会得到一团看似高级、实际没法落地的概念雾。
