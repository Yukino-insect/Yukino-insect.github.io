...
一个典型的 uni-app 前端工程就是这样的结构：业务代码最终运行在 H5、小程序或 App 目标平台里，但开发命令、构建命令、类型检查和依赖管理都通过 Node.js 与 npm 完成。
...
团队协作时，除了锁定依赖版本，还应该尽量锁定 Node.js 和包管理器版本。否则同一份代码在不同机器上可能出现“你能装、我不能装；你能构建、CI 不能构建”的局面。
...
这不是为了显得讲究，而是为了让构建结果可重复。工程问题里最麻烦的往往不是“必现错误”，而是“只有某个人电脑上会发生”。这种问题一旦进了团队协作，就很容易消耗不必要的时间。
